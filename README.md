...
Command Copilot answers plain-English questions with rule-based replies drawn from corridor stats and ASTraM data — no LLM generation in the current build.
...
### Command Copilot — rule-based briefing assistant
A conversational interface with **stub replies** grounded in ASTraM + BTP data. Answers plain-English questions like:
...
Replies are rule-matched from corridor stats, not generated by an LLM. An `api/copilot` server route is pre-wired for Lovable AI Gateway integration but is **not currently called by the UI**.
...
│  Rule-based Copilot replies (stub matching)                 │
└─────────────────────────────────────────────────────────────┘
                             ▼
┌─────────────────────────────────────────────────────────────┐
│                   PRESENTATION LAYER                        │
...
| **Copilot** | Rule-based stub replies (client-side). `api/copilot` server route pre-wired for Lovable AI Gateway — not currently invoked by UI |
